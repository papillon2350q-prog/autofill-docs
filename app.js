...
function recalculateDates() {
  ...
  // Statement Date = fecha base (formato bonito)
  const statement = formatLong(base);
  ...
  const paymentDue = formatLong(due);

  // Expected Graduation → formato corto "May 2026"
  let gradYear = y + 1;
  const grad = new Date(gradYear, 4, 1); // mayo
  if (grad > maxDate) grad.setFullYear(2027);
  const monthsShort = ['January','February','March','April','May','June',
                       'July','August','September','October','November','December'];
  const expectedGrad = monthsShort[grad.getMonth()] + ' ' + grad.getFullYear();

  // Fecha debajo de PROCESSED = misma que Statement (o +0)
  const processedStr = formatLong(base);
